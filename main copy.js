...
// Add passport to request route
// Must be after urlencoded (bodyparser)
app.use(passport.initialize()); 

// Get passport to use session
app.use(passport.session())

// session-based = close browser
app.use('/secure', 
    (req, resp, next) => {
...
// if deleted token (new session; not login yet), will go back to index.html
...
//         .then (result => {
//             if (result)
//                 return resp.status(201).type('text/html').send('hello');
//             resp.status(401).type('text/html').send('incorrect login');
//         })
//         .catch(error => {
//             resp.status(500).type('text/html').send(error);
//         })
